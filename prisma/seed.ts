// Seeds the admin login + demo patients matching the approved design.
// Run: npm run db:seed
...
async function main() {
  ...
  if ((await db.patient.count()) > 0) {
    console.log("Patients already exist — skipping demo seed.");
    return;
  }
  ...
  const patients: Array<{
    ...
    notes: string;
    activity: Array<[number, string]>; // [ms ago, text]
    paidAgoMs?: number;
    paymentType?: string;
  }> = [
  ...
  // Demo-only: every seeded patient can log in at /login with "dental123"
  ...
  console.log("Seeded admin (admin@dentalscotland.com / dental123) + 9 demo patients.");
}
